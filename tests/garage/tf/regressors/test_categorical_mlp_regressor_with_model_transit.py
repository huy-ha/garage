"""
    Unit test for Categorical MLP Regressor with Model.

    This test consists of two different regressors: bmr1, bmr2
    bmr1 is a CategoricalMLPRegressor, which does not use
    ...
    CategoricalMLPRegressor to CategoricalMLPRegressorWithModel.
...
class TestCategoricalMLPRegressorWithModelTransit(TfGraphTestCase):

    def test_all(self, input_shape, output_dim):
        n = 50
        xs = np.random.random((n, ) + input_shape)
        ys = np.random.randint(0, 1, (n, output_dim))

        bmr1 = CategoricalMLPRegressor(input_shape=input_shape,
                                       output_dim=output_dim,
                                       name='bmr1')
        bmr2 = CategoricalMLPRegressorWithModel(input_shape=input_shape,
                                                output_dim=output_dim,
                                                name='bmr2')
