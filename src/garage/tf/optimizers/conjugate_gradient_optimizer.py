...
This optimizer uses conjugate gradient method to compute search direction,
and then computes the optimal step size that will satisfy the KL divergence
constraint. Finally, it performs a backtracking line search to optimize the
objective.
...
class HessianVectorProduct(abc.ABC):
    ...
    def build_eval(self, inputs):
        ...
        Returns:
            function: This function can be called to get the final result.
        ...
        def _eval(v):
            ...
            """
            xs = tuple(self.target.flat_to_params(v, trainable=True))
            ret = sliced_fun(self.opt_fun['f_hx_plain'], self._num_slices)(
                inputs, xs) + self.reg_coeff * v
            return ret

        return _eval


class PerlmutterHvp(HessianVectorProduct):
    """This class computes hessian-vector product using Pearlmutter's algorithm.
...
class FiniteDifferenceHvp(HessianVectorProduct):
    """This class computes hessian-vector product using finite difference method.
    ...
    def update_opt(self, f, target, inputs, reg_coeff, name=None):
        ...
        with tf.name_scope(name, 'FiniteDifferenceHvp',
            ...
            def f_hx_plain(*args):
                ...
                Args:
                    args (tuple(numpy.ndarray)): This tuple contains inputs
                        of function f, and vector v.
...
class ConjugateGradientOptimizer(Serializable):
    ...
    Args:
        ...
        max_backtracks (int): Max number of iterations for backtrack
            linesearch.
        debug_nan (bool): if set to True, NanGuard will be added to the
        ...
        hvp_approach (garage.tf.optimizers.conjugate_gradient_optimizer.HessianVectorProduct): # noqa: E501
            A class that computes hessian-vector
                products.
        num_slices (int): hessian-vector product function's inputs will be
            divided into num_slices and then averaged together to improve
            performance.
    ...
    def __init__(self,
                 ...
                 hvp_approach=None,
                 num_slices=1):
        Serializable.__init__()  # pylint requires to call super init
